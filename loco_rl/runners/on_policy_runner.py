...
import loco_rl
from loco_rl.algorithms import PPO, Distillation, LocoPPO
...
class OnPolicyRunner:
    ...
    def __init__(self, env: VecEnv, train_cfg: dict, log_dir: str | None = None, device="cpu"):
        ...
        if self.alg_cfg["class_name"] == "PPO" or self.alg_cfg["class_name"] == "LocoPPO":
        ...
        # get the number of observations, which is the last dimension of the observation tensor
        # [num_envs, num_obs] or [num_envs, num_history, num_obs] for recurrent policies
        num_obs = obs.shape[-1]
        ...
        policy: ActorCritic | ActorCriticRecurrent | ActorCriticVae | StudentTeacher | StudentTeacherRecurrent = policy_class(
        ...
        self.alg: PPO | LocoPPO | Distillation = alg_class(policy, device=self.device, **self.alg_cfg, multi_gpu_cfg=self.multi_gpu_cfg)
    ...
    def learn(self, num_learning_iterations: int, init_at_random_ep_len: bool = False):  # noqa: C901
        ...
        if self.log_dir is not None and self.writer is None and not self.disable_logs:
            ...
            if self.logger_type == "neptune":
                from loco_rl.utils.neptune_utils import NeptuneSummaryWriter

                self.writer = NeptuneSummaryWriter(log_dir=self.log_dir, flush_secs=10, cfg=self.cfg)
                self.writer.log_config(self.env.cfg, self.cfg, self.alg_cfg, self.policy_cfg)
            elif self.logger_type == "wandb":
                from loco_rl.utils.wandb_utils import WandbSummaryWriter
        ...
        for it in range(start_iter, tot_iter):
            ...
            with torch.inference_mode():
                for _ in range(self.num_steps_per_env):
                    ...
                    # store normalized actor-observations
                    infos["observations"]["next_obs"] = obs
